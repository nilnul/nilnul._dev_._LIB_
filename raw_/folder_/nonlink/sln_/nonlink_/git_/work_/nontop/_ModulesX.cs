...
namespace nilnul.dev.raw_.nonlink.sln_.nonlink_.git_.work_.nontop
{
	static public class _ModulesX
	{
		...
		)
		{
			...
			)
			{
				...
				{
					foreach (var item1 in raw.div_.nonlink_.git_.work_.top._ModulesX.Seq_disregardRepo(item))
					{
						yield return item1;
				...
				{
					foreach (var item1 in raw.div_.nonlink_.git_.work_.nontop._ModulesX.Seq_disregardRepo(item))
					{
						yield return item1;
