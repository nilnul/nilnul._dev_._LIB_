...
namespace nilnul.dev.raw.synd_.cs.doc_.snk_.unoccupied.create_
{
	/// <summary>
	...
	static public class _SymX
	{
		static public string Address_underPrimarySrc() {
			var primary = nilnul.dev.raw_.Primary.Address_ofCfg().ToString();
		...
		)
		{
			...
			nilnul.fs.file_.link_.sym_.path._CreateX.CreateUsingPath(
				...
				System.IO.Path.Combine(prj, nilnul.dev.raw.synd_.cs.doc_._SnkX.TMP_DOC)
				)
				,
				new fs.address_.Spear(
				sharedInsallation
