...
namespace nilnul.dev.raw.prj.asDoc.main
{
	/// <summary>
	/// </summary>
	///
	[Obsolete(nameof(prj.main._NominaX))]
	static public class _NominaX
	{
		...
		)
		{
			...
			if (mainSegs is nilnul.fs.folder._repoName.dir.name_.idioms0Comments_.Segs segs11)
			{
				...
				return nilnul.objs.op_.unary_._AppendX.Append(
					torsoToe.Item1.Select(s=>s.ToString())
					,
					nilnul.txt_.vered._KeyX._Txt(
						torsoToe.Item2.ToString()
					)
				);
		...
		/// 
		/// </summary>
		/// <param name="parentDoc">main -- the part of file anme before  ext</param>
		/// <returns></returns>
		public static string CsNominaOfSemina(string parentDoc)
		{
			return NominaAsTxt(
				nilnul.dev.raw.sln.sub.synd.prj_.cs._meta._LocateX.SpearNulable_ofMain(parentDoc)
			);
		}
