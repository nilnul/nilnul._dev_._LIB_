...
namespace nilnul.dev.raws.prj._locator
{
	...
	/// </summary>
	/// <see cref="nilnul.dev.raws.prj._locator._SlnSubX"/>
	///
	[Obsolete(nameof(dev.raw.sln.sub.dir_._synd._ChooseX))]
	static public class _SubDirX
	{
		...
		static public (nilnul.fs.address_.shield_.BaseDiv, string) _SubDnt_ofMain(
			...
			var slnAsDnt2parsed = nilnul.dev.raw.sln._NameX._Name_assumeSln(slnAsDenote);


			var subSinceSrc =   sub.descendent.dirs;

			var repoName = slnAsDnt2parsed.asSegs().Select(s=>s.name).Concat(
				nilnul.dev.raw.sln.div._RepoNameX.Ids(subSinceSrc)
			);
			...
			var dirCandidates_inDnts = nilnul.fs.folder._DirsX.Captions(sub).Select(
				dnt => (dnt: dnt, segs: nilnul.dev.raw.div.dir.name._ParsedX.ParsedNulable(dnt))
			).Where(
				t => !(t.Item2 is null)
