...
namespace nilnul.dev.raw.sln.sub.of_
{
	/// <summary>
	...
	static public class _IdentityX
	{
		...
		static public nilnul.fs.address_.shield_.BaseDiv _Sub_ofIdentity(
			...
			var rtSln1 = dev.raw.sln.of_._SubX._Sln(srcs, identityOfSub);

			var subAsSegs = identityOfSub.Split(nilnul.obj_.deV._identy._nomina._sur._SepX.Character);


			/// the name of the  sln file.
			var slnName = nilnul.dev.raw.sln._NameX._Name_assumeSln(rtSln1.child.denote.en).asSegs().Select(seg => seg.name);
