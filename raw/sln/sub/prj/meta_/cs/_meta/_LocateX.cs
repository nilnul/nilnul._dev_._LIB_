...
namespace nilnul.dev.raw.sln.sub.synd.prj_.cs._meta
{
	/// <summary>
	...
	static public class _LocateX
	{
		...
		{
			return raw.sln.sub.prj.meta_._ChoosePrjX.ChooseMain(
				new nilnul.fs.Folder(folder), prjIdentity,

				nilnul.dev.raw.prj_.Cs.EXT
			);
		}
		...
		{
			return raw.sln.sub.prj.meta.of_.doc_.commented_._FroManyX.Spears_ofDoc(
				main
				,
				nilnul.dev.raw.prj_.Cs.EXT
			).SingleOrDefault();
		...
		{
			return raw.sln.sub.prj.meta_._ChoosePrjX.Spear_ofDoc(
				main
				,
				nilnul.dev.raw.prj_.Cs.EXT
				,
		...
		{
			return Spear_ofMain(main, nilnul.dev.Srcs_addresses.OvCfg());
		}
