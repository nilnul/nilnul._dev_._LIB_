...
namespace nilnul.dev.raw.sln.sub.prj.meta.nuly.of_.doc_
{
	/// <summary>
	/// choose the project file according to commentedId(nom and version); here doc means not only identity( nomina and version)
	/// 
	/// </summary>
	/// <remarks>
	/// for identity, <see cref="nilnul.dev.raw.sln.sub.synd.prj._main._LocateNulableX"/>
	/// </remarks>
	static public class _LocateNulableX
	{
			...
			return nilnul.objs_._MaxesX.Maxes(
				...
				>(
					...
					new nilnul.obj.comp_.ByTag<
						...
						int
					>(
						t => t.Item2.asSegs().Last().name == "_LIB4FX_" ? 1 : 0
						,
					...
					new nilnul.obj.comp_.ByTag<
						...
						int
					>(
						t => t.Item1.ToString() == commented ? 1 : 0
						,
						Comparer<int>.Default
					)
					,
		...
		static public (fs._address.doc_._exted.Main, fs.folder._repoName.dir.name_.idioms0Comments_.Segs) ChooseMain(nilnul.fs.address_.ShieldI folder,string cot, string[] prjIdentity, string ext)
		{
			return ChooseMain(
				new nilnul.fs.Folder(folder),cot, prjIdentity, ext
			);
		}
		public static (fs._address.doc_._exted.Main, fs.folder._repoName.dir.name_.idioms0Comments_.Segs) ChooseMain(
			nilnul.fs.address_.ShieldI prjFolderParent
			,string cot
			, string nomina
			, string ext)
		{
			//var nomina = nilnul.lang_.cognom._TexX.Parse(cot);
			return ChooseMain(
		...
		public static (fs._address.doc_._exted.Main, fs.folder._repoName.dir.name_.idioms0Comments_.Segs) ChooseMain(nilnul.fs.address_.ShieldI prjFolderParent
			,string cot
			//, string prjNomina
			, string ext)
		{
		...
		public static fs.address_.spear_.ParentDoc SpearNulable_ofDoc(fs._address._route.division_._div._dir.dnt_._exted.Stub stub, fs._address._route.division_._div._dir.dnt_._exted.ExtI ext)
		{
			return SpearNulable_ofDoc(stub,ext.ToString());
		}

		public static fs.address_.spear_.ParentDoc SpearNulable_ofDoc(Exted exted)
		{
			return SpearNulable_ofDoc(exted.stub,exted.ext);
		}


		public static nilnul.fs.address_.spear_.ParentDoc SpearNulable_ofDoc(string main)
		{
			return SpearNulable_ofDoc(
				nilnul.fs._address._route.division_._div._dir.dnt_.Exted.Parse(main)
		...
		{
			main = dev.raw.sln._nomina.Updates.Defaulted.rewritePrj(main);

			var prjAsShield = nilnul.dev.raw.sln.sub.dir_._synd._ChooseX.ShieldNulable_ovBase(
				main
			);
