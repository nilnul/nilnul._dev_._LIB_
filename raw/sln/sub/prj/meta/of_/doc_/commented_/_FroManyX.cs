...
namespace nilnul.dev.raw.sln.sub.prj.meta.of_.doc_.commented_
{
	/// <summary>
	/// choose the project file according to identy(nomina and version)
	/// </summary>
	/// <seealso cref="nilnul.dev.raw.sln.sub.synd._prj._LocateX"/>
	static public class _FroManyX
	{
		...
		public static IEnumerable<nilnul.fs.address_.spear_.ParentDoc> Spears_ofDoc(
			string main
			,
			nilnul.fs._address.doc_._exted.Ext ext, Srcs_addresses srcs
		)
		{
			var cognom = nilnul.lang_.cognom._TexX.Parse(main);
			var nomina = cognom.cognomen;


			nomina = dev.raw.sln._nomina.Updates.Defaulted.rewritePrj(nomina);

			var prjAsShieldS = nilnul.dev.raw.sln.sub.dir_._prj.choose_._ManyX.Shields_assumeNomina(
				nomina, srcs
			);
			foreach (var prjAsShield in prjAsShieldS)
			{
				...
				//	ext
				//);
				var prjMainFound = nilnul.dev.raw.sln.sub.prj.meta.nuly.of_.doc_._LocateNulableX.ChooseMain(
									prjAsShield
									,
