...
namespace nilnul.dev.raw.sln._sub
{
	/// <summary>
	...
	static public class _NulableOfPrj_allowSlnsX
	{
		...
		)
		{
			...
			var rtSlnEs = dev.raw.sln.of_.prj_.nomina_._RetNulableX._SrcSlnEs_ofMain(srcs, main);

			foreach (var rtSln in rtSlnEs)
			{

				var prjName_as_slnSub9tgt = nilnul.dev.raw.sln.prj.name._SlnSub9TgtX.SlnSub9Tgt(main);

				/// the name of the   sln file.
				var slnName = nilnul.dev.raw.sln._NameX._Name_assumeSln(rtSln.child.denote.en).asSegs().Select(seg => seg.name);
				...
				)
				{
					...
					var dir4sub = nilnul.fs.folder._DirsX.Captions(
						...
						nilnul.dev.raw.sln.dir.name._ParsedX.NulableParsed(c)
