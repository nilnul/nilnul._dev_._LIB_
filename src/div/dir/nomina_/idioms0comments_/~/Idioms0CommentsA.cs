...
namespace nilnul.fs.folder._repoName.dir.name_
{
	...
	/// </summary>
	///
	//[Obsolete(nameof(dev.src.div.dir.nomina_.Idioms0commentsA))]

	[Obsolete(nameof(nilnul.lang_.cognom._tex.symbol_.MainX))]
		...
		static public Idioms0CommentsI Parse(nilnul.obj.slider_.Enumer<dev.sln.as_.dst.name._infixer.parener._dot_._lex.WordI> slider)
		{
			...
			switch (slider.current)
			{
				...
				case dev.sln.as_.dst.name._infixer.parener._dot_._lex.word_.TxtOrCommentI txtOrComment:
					...
					//if the begining is not containing any txt, it must be pure comment. if it is pure comment, then no dot shall be followed, and the whole must be comment
					{
						nilnul.obj.vow_.True.Singleton.vow( slider.current ==null, new nilnul.exception_.ParseException("expecting end aft pure comments"));
						return new idioms0Comments_.Comments(
