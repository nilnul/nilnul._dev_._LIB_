...
namespace nilnul.dev.src.prj_.cs.pres_.raw
{
	/// <summary>
	...
	static public class _ClosureOfIdentityX
	{
		...
		)
		{
			...
			_closure._AsynX.Asyn_assumePrj(prjMeta, srcs, nodes, edges).Wait();
...
nilnul.txt.Eq.Singleton,
		...
		static public nilnul.rel_.Net1<string> Net_assumePrjMeta1(
			string prjMeta
			,
			Srcs_addresses srcs
		...
		)
		{
			...
			_closure._AsynX.Asyn_assumePrj1(prjMeta, srcs, nodes, edges).Wait();
...
nilnul.txt.Eq.Singleton,
 ...
 ,srcs
		...
		)
		{
			...
			_closure._AsynX.Asyn_assumeIdenty(prjNomina, srcs, nodes, edges).Wait();
