namespace nilnul
{
	/// <summary>
	/// first the static structure, which is a state at a timepoint, then the dynamic behavior, which is a bi-tuple: [startState, end state] describing the change across a timespan;
	/// </summary>
	/// <remarks>
	/// treating the development itself as a system to development: including:
	///		,catering to its demand,
	///		,tooling
	/// </remarks>
	/// alias:
	///		dev
	///			,delve
	/// vs:
	///		zen, which abstracts the dev.zen, whileas this is only zen about dev.
	///		
	public interface IDev { }

}
