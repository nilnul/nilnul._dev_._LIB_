...
According to a previous answer, 2017 does in fact store this information in the registry.*/
	...
	/// let's put the build here, not in dev.src as it's for src code management(structure, git repo,etc) and the primary tool there is git.exe, not in nilnul.dev.pak as it's for nuget package management(pak, push, install, versions match resolving, etc) and the primary tool there is nuget.exe;
	///
	/// <summary>
	/// msbuild, the toolset for the environment where we can develop;
	/// </summary>
	/// <remarks>
	///	 vs Compile:
	///		compile src files
	...
	///
	/// 
	///  some core concepts are defined in this project.
	...
	public class MsBuild: nilnul.win.Prog
	{
		...
		static public nilnul.fs.address_.SpearI Path() {
			return new nilnul.fs.address_.spear_.ParentDocument(
				prog_.VisualStudio.Yr2022Community
				,
				Denotation
			);
		}
