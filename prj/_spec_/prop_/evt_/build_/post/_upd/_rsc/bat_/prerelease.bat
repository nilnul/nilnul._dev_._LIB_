...
if $(ConfigurationName) == Release (
	...
	if  $(ConfigurationName)==Symbol (
		...
		"NuGet.exe" pack "$(ProjectPath)" -Symbols -Prop Configuration=$(ConfigurationName) -Exclude **/*.tt -Version @(VersionNumber)-$(nilnulTimestamp)
		"NuGet.exe" push $(TargetName).@(VersionNumber)-$(nilnulTimestamp).symbols.nupkg -Source local
		"NuGet.exe" push $(TargetName).@(VersionNumber)-$(nilnulTimestamp).symbols.nupkg -Source my
		rem // change my to a usable one.
