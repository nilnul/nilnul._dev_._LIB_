...
if $(ConfigurationName) == Release (
	...
	if  $(ConfigurationName)==Symbol (
		"NuGet.exe" pack "$(ProjectPath)" -Symbols -Prop Configuration=$(ConfigurationName) -Exclude **/*.tt -Version @(MajorMinor).$(nilnulDate).$(nilnulTotalSecondsTrunc)
		"NuGet.exe" push $(TargetName).@(MajorMinor).$(nilnulDate).$(nilnulTotalSecondsTrunc).symbols.nupkg -Source local
		"NuGet.exe" push $(TargetName).@(MajorMinor).$(nilnulDate).$(nilnulTotalSecondsTrunc).symbols.nupkg -Source my
	)
