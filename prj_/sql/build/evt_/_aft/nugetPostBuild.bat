...
@rem		set /a p=year*366+month*30+day
		set  p=%year%%month%%day%

		echo patch: %p%
@rem		set /a b=(hour *3600+min*60+sec)/2
		set  b=%hour%%min%%sec%
@rem /a means algrebra; this would trim out leading zeros;
		set /a b=b
		if b==0 ( 
			set b=1 
		)
...
@rem : change the follows ----------------------------------
		set pkg=$packageNomina$.%v%.nupkg
		echo %pkg%
		nuget pack -prop configuration=Release -Version %v%
@rem change the above configuration-------------------------------------
		"NuGet.exe" push %pkg% -Source local
		"NuGet.exe" push %pkg% -Source my
